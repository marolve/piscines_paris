...
function fillHtml(html, data) {
	...
	let row = '\n					<tr %schedulelabels% %scheduledatas%> <td>%arr%</td> <td><a href="%link%">%name%</a></td> <td class="schedulecell"></td> </tr>';
	for (const piscine of data.piscines) {
		if (piscine.schedules.length > 7) {
			...
			piscineRows += row.replace('%arr%', piscine.arrondissement).replace('%name%', piscine.name).replace('%link%', piscine.link).replace('%schedulelabels%', scheduleLabels).replace('%scheduledatas%', scheduleDatas);
		}
	};
